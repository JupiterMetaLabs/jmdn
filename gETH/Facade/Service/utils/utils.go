...
// Calculate the base
func calculateBaseFee(ZKBlock *config.ZKBlock) []byte {
    basefee := big.NewInt(0) // Initialize with 0
    for _, tx := range ZKBlock.Transactions {
		
		// Debugging
		fmt.Printf("Gaslimit: %d, Gasprice: %d TransactionHash: %s\n", tx.GasLimit, tx.GasPrice, tx.Hash)

        Gaslimit := big.NewInt(int64(tx.GasLimit))
        Gasprice := tx.GasPrice
		
        if Gasprice == nil || Gasprice.Sign() == 0 {
            continue // Skip if GasPrice is nil or zero
        }

        localbasefee := new(big.Int).Mul(Gaslimit, Gasprice)
        basefee.Add(basefee, localbasefee)
    }
    // Always return at least a zero value if no transactions or all GasPrices were nil/zero
    if basefee == nil || basefee.Sign() == 0 {
        return []byte{0}
    }
    return basefee.Bytes()
}
...
func ConvertZKBlockToblockheader(ZKBlock config.ZKBlock) Types.BlockHeader {
	...
	return Types.BlockHeader{
		...
		BaseFee:             calculateBaseFee(&ZKBlock),
