...
func (h *CommandHandler) StartCLI(ctx context.Context, bindAddr string, grpcPort int) error {
	...
	CLICommon.LocalGRO.Go(GRO.CLIThread, func(ctx context.Context) error {
		...
		if err := StartGRPCServerWithContext(ctx, h, bindAddr, grpcPort); err != nil {
	...
	CLICommon.LocalGRO.Go(GRO.CLIThread, func(ctx context.Context) error {
		...
		if !isInteractive() {
			...
			// In non-interactive mode, wait for context cancellation
			<-ctx.Done()
			return nil
		}
		...
		if !scanner.Scan() {
			// EOF immediately - stdin not actually available, switch to waiting mode
		...
		if input == "stopService" {
			...
			// Use centralized shutdown function for safety
		...
		// If stdin closes later, switch to waiting mode instead of exiting
		for scanner.Scan() {
			input := strings.TrimSpace(scanner.Text())
			if input == "stopService" {
				fmt.Println("stopService")
		...
		// Scanner exited (EOF) - stdin closed, switch to waiting mode
		if err := scanner.Err(); err != nil {
			log.Printf("Scanner error: %v", err)
		}
