...
// arrives. dedupMessageCache is an exact, bounded LRU with entries that can
// age out but are never falsely present, so validate-before-cache remains the
// safe ordering (an entry, once added, is never a false hit for something
// else).
