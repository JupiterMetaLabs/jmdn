...
| **D-67** | 2 | Fingerprint check ran after the block-processed marker and left applied writes on failure → retry took the unverified "already processed" skip path, bypassing the fail-closed gate (`Processing.go:520` vs `:565`) | Check **before** the marker; on failure roll back the applied+marked prefix (revoke per-tx markers, then restore balances, under the state-apply lock) so no marker/state remains and a re-delivery re-verifies | *(unit: failed fingerprint ⇒ no marker + no residual mutation — to add in `messaging/BlockProcessing`)* |
