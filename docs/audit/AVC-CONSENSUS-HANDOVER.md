...
| **D-34** | 3 | `jmdn` | Unbounded maps on the block-receive path (`seenHeights` et al) | — | M | `Partially Fixed — of the four sites this row named: (1) messaging/consensus_hardening.go seenHeights is now bounded (LRU cap 100,000, PR-3 fix/d34-bound-equivocation-cache, e016e1f/76ad6fe) — CLOSED. (2) messaging/blockPropagation.go:187 updateMessageSet's O(n) rewrite is now a no-op (DB_OPs.Create is a stub post-ThebeDB migration, thebe_ops.go:66 — "All other ImmuDB-specific KV entries are no-ops") — the growth risk this row named no longer applies, though whether crdt:message_set data is now silently unpersisted is a separate question this row does not cover. (3) Pubsub/Subscription/SubscriptionManager.go's errored-subscription leak was independently CLOSED by JMDN-V3-012(d) (PR-1, f8096d1/9a75a9e — the same PR that closed D-57) — SubscriptionManager.go:214-222 now deletes the map entry on the error path, race-guarded. (4) Sequencer/vdf_seal_wiring.go:70 vdfSealers is still OPEN by explicit design — CancelSealer's own comment (:138) says the entry is "deliberately NOT removed," and safe eviction depends on epoch finalization being TOCTOU-safe, which D-31 (still Open) puts in question. Do not close this row until (4) is addressed.` |
