...
TALLY (recounted merging feat/v2-vote-crdt-cutover (#155, merged) with
fix/d58-persist-epoch-entropy — 2026-09-24. Reflects all landed PRs up through
#153, #154, #155: D-26 is now fully Fixed across both PRs ((c)/(d) in #154,
(a)/(b) and D-71 in #155), D-31/D-34 landed in PR #153, D-48 responder gate in
PR #154, D-51 and D-71 landed in #155. fix/d58-persist-epoch-entropy adds D-58
Fixed and opens D-72; it also fixes D-73 (evictOldSealersLocked missing
s.Cancel() before delete, not yet execution-verified — see that row).
Recount whenever a row moves: Appendix C.2's
"a register decays fastest in its summary layer".)

  54 rows = 34 Fixed · 4 Partial · 1 Decided-no-change · 15 Open

  Fixed (34)    D-24 D-25 D-26 D-27 D-29 D-30 D-31 D-32 D-33 D-34 D-35 D-36
                D-37 D-39 D-41 D-51 D-54 D-57 D-58 D-60 D-61 D-62 D-63 D-64
                D-65 D-66 D-67 D-68 D-69 D-70 D-71 D-73 D-74 D-77
  ...
  Open (15)     D-40 D-42 D-43 D-44 D-45 D-47
                D-49 D-50 D-52 D-53 D-55 D-59 D-72 D-75 D-76

BY SEVERITY   5 + 13 + 27 + 8 + 1 = 54
  SEV-1   5   D-24✓ D-25✓ D-26✓ D-35✓ D-36✓ | ALL CLOSED — D-26 closed across
              #154 ((c)/(d)) and #155 ((a) + (b) + D-71).
  SEV-2  13   D-27✓ D-29✓ D-37✓ D-39✓ D-51✓ D-64✓ D-67✓ D-68✓ D-70✓ D-74✓ D-77✓
              | OPEN: D-28(detection), D-38(b)
  SEV-3  27   D-30✓ D-31✓ D-32✓ D-34✓ D-41✓ D-54✓ D-57✓ D-58✓ D-60✓ D-61✓ D-62✓ D-65✓ D-66✓
              D-71✓ D-73✓ D-56(decided) | PARTIAL: D-46 | OPEN: D-40 D-42 D-43 D-44 D-45 D-52 D-53 D-59 D-72 D-75
  SEV-4   8   D-63✓ D-69✓ | PARTIAL: D-48 | OPEN: D-47 D-49 D-50 D-55 D-76
  Unrated 1   D-33✓ (remainders open — persistence, observe rung)
  Devnet  9   §6, tracked as a group, NOT in the 51
...
| **D-26** | 1 | `jmdn` | Vote CRDT keyed on a self-declared sender; requester chooses the signing target; guard off and unwired | `avc crdt/votes/write_identity_binding_test.go` | L | `Fixed — (a), (b), (c), (d) ALL CLOSED.` (c) and (d) CLOSED by PR #154; (b) and (a) CLOSED by feat/v2-vote-crdt-cutover (phase 1: own-vote publish, phase 2: drop relay + re-key on authenticated sender; D-71 closed). See the ▶ PR #154 and ▶ feat/v2-vote-crdt-cutover blocks at the end of this cell for evidence; the rev-5/rev-6 text below is kept unedited per §2.1. Historical status follows: Open — avc-side hardening landed, closes nothing in jmdn yet. avc 3eee4b0 (first carried by v0.1.0-v3base.5) makes AddVote REJECT a record whose payload rec.PeerID differs from the libp2p-authenticated nodeID (write.go:74); it does NOT re-key — both elements are still keyed on rec.PeerID (write.go:83/:95/:100, and its own comment at :46 says so). Structurally inert in jmdn today: the sole non-test caller (Vote/Trigger.go:286) sets rec.PeerID from the same identity it passes as nodeID (:268), so the two can never differ, and the whole block sits behind JMDN_VOTE_CRDT_V2 (default OFF). avc also EXEMPTS the merge path (write.go:64-66). D-26(a)''s actual defect site — jmdn legacy pubsub ingest keying the CRDT on msg.Data.Sender in AVC/.../Service/subscriptionService.go — does not call AddVote at all and is untouched. Remainder: (a) legacy ingest, (b) membership filter, (c) signing target, (d) requester-auth guard.` **▶ REV 6 — (a) RE-VERIFIED AND SHARPENED; it is worse than this row said.** Three facts, each executed: (1) **`msg.Data.Sender` is a JSON payload field** (`config/PubSubMessages/Pubsub.go:19`) and **nothing on this path compares it to the authenticated sender** — but note the fix is *cheap*, because **the authenticated identity is already in hand**: `Pubsub/Subscription/SubscriberHelper.go:293` and `SubscriptionManager.go:244` both set `Sender: msg.GetFrom()` on the same struct. Two sender fields, one authenticated and one attacker-chosen, and the handler reads the wrong one. No plumbing is needed — the value is already there. *(An earlier rev-6 note said "0 `ReceivedFrom`/`GetFrom()` anywhere"; that grep was scoped to `AVC/BuddyNodes/MessagePassing/` and missed `Pubsub/`. Corrected — the §3 evidence below had it right all along.)* (2) The CRDT write at `subscriptionService.go:291-300` (`NodeID: msg.Data.Sender`, `Key: msg.Data.Sender.String()`) has **0 gate references** in the whole handler — it is **unconditional and live on a default node**, whereas the hardened v2 path is the one behind `JMDN_VOTE_CRDT_V2` (default OFF). **The unauthenticated path is the default path; the authenticated one is opt-in.** (3) **`RejectLegacyVotes` does not cover this.** Despite the name, it is applied only in `Sequencer/Consensus.go:2344`, `consensus_hardening.go:543` and `committee_v2.go:701` — the **tally/signature** paths, not the **ingest** path. **Bounding the impact honestly:** with `RejectLegacyVotes` on (default), a forged element still needs a valid block-bound BLS signature to count toward quorum, so this is **not** direct quorum forgery today. What it does give an unauthenticated peer is the ability to write arbitrary elements under **any** peer's CRDT key — unbounded (D-34), able to fabricate or destroy equivocation evidence (cf. D-57), and to set the per-peer budget against a victim (D-52). Another RC-3: a guard whose name tells the reader it covers the legacy vote path, which covers the tally instead **▶ PR #154 (2026-09-23) — (c) AND (d) CLOSED.** **(c) signing target — Fixed** (`60cef88`): `handleVoteResultRequest` now resolves the block LOCALLY by `block_hash` through the new `LookupConsensusMessageByBlockHash` (`config/PubSubMessages/Consensus.go`) and **overwrites** `targetBlockNumber`/`targetConsensusHash` with local values before `SignMessageForBlock`; an unknown block is refused. The caller may say WHICH block, never what that block's height or digest IS — and because the local values are what get signed, a hole in the comparison still could not change the signed content. Test: `config/PubSubMessages/d26c_consensus_lookup_test.go`. **(d) requester-auth guard — Fixed** (`68ae9c5` observe rung, then `1c44341`): `voteRequesterAuthorized` is now fail-closed on every path — the `if len(set) == 0 { return true }` fail-open is GONE — `SetAuthorizedRequesterSource` is wired unconditionally at startup from `messaging.EligibleCommitteePeerIDs()` ∪ the new `consensus.sequencer_pinned_peer_id`, and `ValidateProductionConsensusPosture` refuses to boot a production node with that pin unset. `a9410cc` unexports the `voteRequesterAuthorizer` override seam so it cannot be reached from outside the package (D-49-class hygiene, compiler-enforced). Test: `AVC/BuddyNodes/MessagePassing/consensus_vote_authz_test.go`. **`68ae9c5` is the reference implementation of RC-2's missing middle rung** — it measures what the gate WOULD decide while the gate stays off, so promotion is evidence-driven; use it as the template for the other two-state gates. **Scope note:** PR #154's commits also cite `CON-01`/`CON-03`, which belong to `THEBE-AUDIT-HLD.md`'s ID space — a register that is still in no git repo (§0), so those two IDs remain untrackable **▶ feat/v2-vote-crdt-cutover (2026-09-24) — (a) AND (b) CLOSED; D-71 CLOSED.** **(b) membership filter — Fixed** (`5da94b6`): the legacy tally's `weight := 1.0; exists := true` fallback — which applied to **every** key in the CRDT whenever the seed weight map was unavailable, i.e. the normal case on a buddy — now resolves `authorizedCommittee()` and **refuses to tally** if it cannot, instead of silently weighting unknown peers. Wired at `main.go` → `messaging.AuthorizedCommitteeForTally`. **(a) phase 1 AND phase 2 CLOSED:** `f430919` (phase 1) has each voter publish its own vote under its own libp2p-authenticated identity. Phase 2 (`d9f8530`, D-71) drops the relay at `ListenerHandler.go:1147` and re-keys the CRDT write at `subscriptionService.go` onto `msg.Sender` (`msg.GetFrom()`), closing impersonation structurally on an operator-confirmed coordinated fleet restart. |
...
| **D-31** | 3 | `jmdn` | Epoch watermark TOCTOU duplicates finalisation; duplicate seal blocks a goroutine forever | `Sequencer/vdf_sealer_cancel_test.go`, `Sequencer/d31_sealer_eviction_test.go` | M | `Fixed (fix/rpc-timeouts-and-limits) — three sub-claims closed, each verified independently against live code. (1) The EPOCH watermark TOCTOU itself was already closed pre-existing: messaging/entropy_finalise.go's maybeFinaliseCompletedEpochs claims lastDecidedEpoch = e under finaliseTrackMu BEFORE calling decideEpoch outside the lock — confirmed by reading the live code, not re-trusted from this row's own history. (2) Sequencer/vdf_sealer.go's Start checked s.cancel != nil (already-started) but never s.cancelled — fixed: Start now returns immediately, before launching anything, if s.cancelled is set. (3) Sequencer/vdf_seal_wiring.go's vdfSealers had no eviction anywhere outside ClearSealerForTest — fixed: sealerFor and CancelSealer both call the new evictOldSealersLocked, bounded by the same JMDN_AVC_BEACON_RETAIN_EPOCHS-derived window beacon_install.go already computes at startup (committee.MinRetainedEpochs default). Also closes the actual TOCTOU race this row's short description names: CancelSealer used to silently no-op when no sealer existed yet for forEpoch — the exact ordering where a peer's proof is adopted before this node's own onEpochFinalised fires — dropping the cancellation entirely; it now plants a pre-cancelled placeholder so the later Start call refuses to launch a redundant ~T_vdf evaluation, and sealerFor returns that same placeholder rather than a fresh sealer on the later hit. Tests: Sequencer/vdf_sealer_cancel_test.go (extended, 2 new cases), Sequencer/d31_sealer_eviction_test.go (new, 3 cases). Note: the new retention-window read duplicates beacon_install.go's parsing rather than sharing it — this is the same divergence-prone pattern D-47 (still Open, separate finding) already flags for the mix-store window; D-47 is not touched by this fix. AND BEWARE A MIS-TAGGED COMMIT, unchanged from this row's prior text: PR #129's 1624583 is titled "fix(crdt): prevent watermark TOCTOU resurrecting compacted votes (D-31)" but does NOT touch this defect — it edits avcvotes.DefaultWatermark in CRDTSyncHandler.go, the VOTE-COMPACTION watermark, which this register assigns to D-53. 1624583 remains D-53's commit, not this row's, and it also introduced a new defect → D-57 (Fixed, PR-1).` |
...
| **D-34** | 3 | `jmdn` | Unbounded maps on the block-receive path (`seenHeights` et al) | — | M | `Fixed — all four sites this row named are now closed or moot. (1) messaging/consensus_hardening.go seenHeights is bounded (LRU cap 100,000, PR-3 fix/d34-bound-equivocation-cache, e016e1f/76ad6fe). (2) messaging/blockPropagation.go:187 updateMessageSet's O(n) rewrite is now a no-op (DB_OPs.Create is a stub post-ThebeDB migration, thebe_ops.go:66) — the growth risk this row named no longer applies; whether crdt:message_set data is now silently unpersisted is a separate question this row does not cover. (3) Pubsub/Subscription/SubscriptionManager.go's errored-subscription leak was independently closed by JMDN-V3-012(d) (PR-1, f8096d1/9a75a9e — the same PR that closed D-57). (4) Sequencer/vdf_seal_wiring.go:70 vdfSealers is now bounded (fix/rpc-timeouts-and-limits, D-31's fix — see that row): evictOldSealersLocked prunes below the same retention window beacon_install.go uses, and the eviction only became safe to add once D-31's finalization-TOCTOU and duplicate-cancellation fixes closed — evicting before that would have let a false "not ready" or a duplicate ~T_vdf evaluation slip through on a map miss.` |
...
| **D-51** | 2 | `jmdn` | **`JMDN_VOTE_CRDT_V2` makes the entire avc v2 vote keyspace inert.** Every vote hardening avc ships — D-26's identity guard, `MaxElementsPerPeerPerBlock`, the compaction watermark — reaches production only through `Vote/Trigger.go`, gated on `VoteCRDTDualWrite = envOn("JMDN_VOTE_CRDT_V2", false)`. Structurally the same defect as D-38 ("D-28's fix ships inert") but for a whole keyspace, and it is the reason D-26's avc-side fix closes nothing. RC-2 with no observe rung | `Vote/vote_crdt_v2_test.go` | M | `Fixed (feat/v2-vote-crdt-cutover, cf624c1)` — `VoteCRDTDualWrite` (`Vote/vote_crdt_v2.go`) and `voteCRDTV2Enabled` (`Structs/Utils.go`) are hardcoded `true`, not env-gated, so `ProcessVotesFromCRDT` always routes to `processVotesFromCRDT_v2` → `avcvotes.TallyBlock` → `verifyTallySignatures`. Every hardening avc shipped — the identity guard, `MaxElementsPerPeerPerBlock`, the compaction watermark — is live rather than inert. **⚠ RC-2 NOTE — this left the ladder by DELETING a state, which §7.3 calls legitimate for a *format* and not for *behaviour*.** There is no env override and no observe rung: rollback is a binary rollback plus a fleet restart. The cutover is also asymmetric across a mixed fleet — see **D-71**, opened by this change — so "one coordinated restart" is an operational assumption this code neither enforces nor detects. Taken deliberately; recorded here so the next reader does not mistake it for an oversight |
...
| **D-57** | 3 | `jmdn` | **The D-31-tagged fix erases equivocation evidence.** `1624583` closes a real TOCTOU (a merge can re-create a key the compactor just deleted) by re-checking the watermark after the writes and, if it moved, calling `CRDTLayer.Delete(key)` — **the whole LWWSet, not just the elements this merge added** (`CRDTSyncHandler.go:860`). If that key already held a peer's genuine conflicting votes, the proof is destroyed before `ConvergeAndCompact`'s C5 pass evaluates it — the exact ordering `avc crdt/votes/converge.go` says the two steps were fused to prevent, and `CompactVotesBelowHeight`'s own doc warns against. `ReportEquivocation` never fires, the reputation event and metric are lost, and the operator log reports the deletion as a *successful defence*. The delete is unconditional on the watermark condition, so it does not require that anything merged: a peer can send `{"adds":{}}` and still trigger it, retrying near each watermark advance to make honest nodes erase proof of a victim's equivocation | — | S | `Fixed — 1624583's unconditional CRDTLayer.Delete(key) on watermark re-check removed (f8096d1, PR-1). Matches this row's own recommended fix (option 1: drop the delete entirely — not load-bearing, CompactVotesBelowHeight collects the key on the next sweep anyway, after C5 evaluates evidence). No regression test added yet for the two-goroutine race this row asked for.` |

| **D-58** | 3 | `jmdn` | **Epoch-entropy persistence failures are silently discarded at both write sites.** `PersistEpochEntropy` (`messaging/entropy_persist.go:37`) is the durability half of D-33's remainder-1 fix, and **both** its call sites drop the error: `Sequencer/vdf_sealer.go:123` `_ = messaging.PersistEpochEntropy(forEpoch)` (after a successful seal) and `messaging/entropy_vdf_accept.go:199` `_ = PersistEpochEntropy(declaredEpoch)` (after accepting a peer's proof). A node that seals or adopts an epoch, fails to write it, and restarts has **no record it ever held that epoch and no signal it lost one** — `RehydrateBeaconFromDisk` simply restores fewer epochs than it should, reporting success | — | S | `Fixed (fix/d58-persist-epoch-entropy) — both halves of this row's own prescribed fix are in.` **(1) "log at ERROR with the epoch"** — satisfied and re-verified: the log sits inside `PersistEpochEntropy` itself, so BOTH call sites emit it; the accept path's `_ =` discards the return value, not the diagnostic. **(2) "on the sealer path, treat a persist failure as a seal failure"** — `21a4bef` gates `SealResult.Err` on it. **⚠ (2) WAS INERT AS FIRST WRITTEN, and repairing that is the substantive part of this fix.** `PersistEpochEntropy` returned plain `nil` whenever the beacon did not hold the epoch, so the sealer's new gate could only ever fire on a KV *write* error — never on the "sealed but nothing landed" case this row is actually about. A sentinel, `messaging.ErrNoEntropyToPersist`, now separates "nothing to persist" from success, which is what makes the gate real. **Note the mechanism, because the obvious reading is wrong:** `committee.BeaconSource.EpochEntropy` is a map lookup under an RLock — `activeBeacon()` returns that concrete type, not an interface — so it performs no I/O and has exactly one error path. There is no "read failure" to distinguish; the distinction that matters is WHICH CALLER is asking. The accept path stays non-escalating, exactly as this row words it ("on the sealer path"): it is synchronous inside block acceptance, so escalating would turn a local storage hiccup into a fleet-wide block rejection. **Regression cover:** `messaging/entropy_persist_sentinel_test.go` — three tests pinning that an absent epoch returns the sentinel rather than nil, that the sentinel survives wrapping (`errors.Is` reaches both it and the underlying `committee.ErrEntropyUnavailable`, which is what the sealer's switch depends on), and that Stage 1 with no beacon installed stays a silent nil. **Confirmed to be real cover, not decorative:** reverting the sentinel to `return nil` makes the first two fail. **Owed, not blocking:** this row's own `Verify` clause (force the KV write to fail, restart, assert the node names the lost epoch) is still NOT executed — `DB_OPs.RecordBeaconEntropy` needs a live ThebeHandle and no restart harness exists in this package, so that end of it remains untested. **Related, and NOT closed by this:** the sealer persists via a process-global beacon rather than its own pipeline sink — **D-72**, opened by this work. **Severity is LIVENESS, not divergence — and only because D-25 is now fixed.** Post-D-25 the missing epoch makes `SeedSourceFor` return `ErrBeaconEpochUnavailable`, so the node refuses to select and halts loudly instead of seating a different committee; before D-25's fix this same loss was a silent fork. **Fix:** log at ERROR with the epoch and, on the sealer path, treat a persist failure as a seal failure — the entropy is worthless to this node after a restart if it was never written. Cheap: two call sites. **Verify:** force the write to fail, restart, assert the node names the lost epoch rather than reporting a clean rehydrate. Related, not duplicate: **D-47** (the restore *window* is a compile-time constant that `JMDN_AVC_BEACON_RETAIN_EPOCHS` does not move) and **D-45** (the newest-pointer RMW that can strand a written epoch) |
...
| **D-71** | 3 | `jmdn` | **D-26(a) phase 2 — re-key the vote CRDT on the authenticated sender, and drop the relay that blocks it.** Phase 1 (`f430919`, feat/v2-vote-crdt-cutover) has each voter publish its own vote under its own libp2p identity, so on a fleet fully on phase 1 `msg.Data.Sender == msg.GetFrom()` for every honest message. Phase 2 is the part that actually closes D-26(a): **(i)** drop the direct-stream→pubsub republication at `ListenerHandler.go:1147`, which is the ONLY reason the two fields legitimately differ (it republishes under the RELAYER's identity while the payload keeps the original voter — this is why the naive ingest guard was tried in `617dd0e` and reverted in `4d621ea`/`98ce387`, and why the D-26(a) note in PR #149 exists); **(ii)** re-key the CRDT write at `subscriptionService.go:304`/`:307` from `msg.Data.Sender` onto `msg.Sender`, making impersonation structurally impossible instead of merely checked. **PRECONDITION — the whole fleet must be on phase 1 first.** Dropping the relay before every node publishes its own vote silently loses the votes of nodes that still rely on relay; re-keying before that makes them unattributable. This is a two-release sequence, not one change. **Severity note:** rated 3 rather than inheriting D-26's 1 because the register's own bound still applies — `RejectLegacyVotes` plus the v2 tally's BLS check mean a forged element cannot reach quorum; what remains is the ability to write arbitrary elements under any peer's key, which is the feeder for D-34 (unbounded growth), D-52 (per-peer budget burned against a victim) and D-57-class evidence tampering, all themselves SEV-3. **Until this lands, phase 1 also ships a permanent +1 pubsub amplification per vote** — the relay is retained but no longer needed for honest traffic | — | M | `Fixed (feat/v2-vote-crdt-cutover) — opened and closed in the same branch, deliberately: the row exists so the PRECONDITION is on the record, not because the work was deferred.` **(i)** the republish block in `handleSubmitVote` (`ListenerHandler.go`) is deleted, and the `Publisher` import with it. **(ii)** `subscriptionService.go` now builds the CRDT `OP` with `NodeID: msg.Sender` / `Key: msg.Sender.String()` — the libp2p-authenticated publisher (`msg.GetFrom()`, set at `SubscriberHelper.go:293` / `SubscriptionManager.go:257`) — instead of the self-declared `msg.Data.Sender`. A peer can no longer write under another peer's key at all, rather than being checked and rejected. **⚠ SCOPE — read this before citing the row as a quorum fix.** The keyspace this protects is the LEGACY CRDT layer (`listenerNode.CRDTLayer`), which the pubsub receive path writes. Post-cutover the tally reads the **v2** layer (`listenerNode.VoteCRDTLayer`, `Structs/Utils.go:228`), fed only by `avcvotes.AddVote` and the CRDT-sync merge — so the quorum-forgery path was already closed by the v2 tally's BLS check, not by this. What this closes is the CRDT-POLLUTION surface: arbitrary elements written under a victim's key, which is the feeder for **D-34** (unbounded growth), **D-52** (per-peer budget burned against a victim) and **D-57**-class evidence tampering — all still live, which is why this is worth doing rather than deferring to the tally check alone. `msg.Data.Sender` is now only a claim: it is read nowhere, but a disagreement with `msg.Sender` is logged at WARN, since post-phase-2 there is no honest reason for the two to differ. The self-vote dedup check and the handler's log fields were re-pointed at `msg.Sender` in the same change so one identity is used throughout. **⚠ SHIPPED ON AN OPERATOR-CONFIRMED COORDINATED RESTART.** The precondition this row was created to record — every node on phase 1 — is not verified by any code; a node still sending direct-stream-only has no republisher now, so its vote reaches one peer and is invisible to the rest of the fleet with no error anywhere. This rode the same fleet-wide restart the v2 read cutover already required. **Do not re-introduce a direct-stream-only vote path without restoring a relay with it.** Residual closed as a side effect: phase 1's +1 pubsub message per vote is gone |
| **D-72** | 3 | `jmdn` | **The sealer persists from a process-global beacon, not from the sink it actually sealed into.** `VDFSealer` holds its own `*beacon.Pipeline`, and `Pipeline.Seal` publishes the entropy into *that pipeline's* `sink`. But the durability step calls `messaging.PersistEpochEntropy(forEpoch)`, which reads the package-global `beaconSource` set by `messaging.SetBeaconSource`. **Production wires them to the same object and nothing enforces it:** `Sequencer/beacon_install.go` constructs one `sink`, hands it to `beacon.New(group, difficulty, sink)` and then to `messaging.SetBeaconSource(sink)` — an invariant held only by those two lines sitting near each other. A `VDFSealer` built with any other pipeline persists against a beacon it never wrote to. **Not hypothetical:** this is exactly what the `Sequencer` package's own tests do (`vdf_sealer_test.go` builds `sinkA`/`sinkB` and never touches the global), and it surfaced as three tests failing the moment D-58's sentinel made the mismatch observable — `TestSealingIsDeterministic`, `TestResultDeliversAfterCompletion`, `TestDuplicateStartStillDeliversOneResult`. Note they pass in isolation and fail in a full package run, so this is order-dependent and was invisible until the sentinel landed. **CONTAINED, NOT FIXED.** D-58's fix distinguishes the two cases at the call site with `s.pipeline.Ready(forEpoch)` — which interrogates the sealer's OWN sink — and downgrades a global-vs-sink mismatch to a WARN rather than failing the seal. That stops the false positive; it does not remove the coupling. **Fix:** thread the sink (or the pipeline) into the persist call so durability reads the same object the seal wrote, and delete the global read from that path. **Verify:** construct a sealer with a sink that is NOT the global, seal, and assert the entropy is persisted from the sealer's own sink rather than warned about | `Sequencer/vdf_sealer_test.go` (the three tests above exercise the divergent-sink shape today) | M | `Open — opened 2026-09-24 by fix/d58-persist-epoch-entropy. Found because D-58's sentinel made a silent mismatch loud; recorded rather than folded into D-58 because the coupling is a distinct defect that outlives that fix.` |
| **D-73** | 3 | `jmdn` | **`evictOldSealersLocked` drops a sealer's map entry without cancelling it first.** `Sequencer/vdf_seal_wiring.go` (pre-fix) did a bare `delete(vdfSealers, e)` for every epoch below the retention cutoff. The map entry is the ONLY reference to that sealer's context cancel func — `CancelSealer` calls `s.Cancel()` before its own delete (`:275`), but eviction never did. If an evicted epoch's `~T_vdf` evaluation is still running, deleting the entry leaves nothing able to stop it, and the result is already unreachable the moment the entry is gone (`sealerFor` refuses to resurrect below `evictedBelow`; `SealerResultFor` reports not-ready once the entry is missing) — so the goroutine burns a core for a result no one can ever read. | `Sequencer/d73_evict_cancels_test.go` (TestEvictOldSealersCancelsBeforeDropping, TestEvictOldSealersLeavesRetainedSealersRunning) | S | `Fixed (fix/d73-vdf-sealer-eviction-cancel) — s.Cancel() now runs before delete(vdfSealers, e), so eviction always releases the handle it's about to drop. **Usually academic, not always:** by the time an epoch falls a full retention window behind, a correctly-sized T has long since finished on its own; this stops being academic under D-46(a)'s exact scenario — T has no upper bound and Start uses context.WithCancel rather than WithTimeout, so a divergent or oversized T (D-39/D-54's own scenario) can still be evaluating at eviction time. Reasoned safe against deadlock: Cancel() takes the sealer's own mutex and invokes a context.CancelFunc, touching neither vdfSealersMu nor the map, and does not block waiting for the goroutine to notice, so calling it while holding vdfSealersMu introduces no new lock ordering. **Does not close D-46(a)** — T is still unbounded; this only ensures eviction can no longer orphan the goroutine evaluating it. Not D-34 (already Fixed, and about map growth, not cancellation) and not the same defect as D-72 (process-global-beacon sink mismatch) — two unrelated findings on the same file/branch lineage, kept separate on purpose. **NOT YET EXECUTION-VERIFIED — do before merging:** \`go test ./Sequencer/... -run TestEvictOldSealers -v -race\`, and confirm reverting the s.Cancel() call makes TestEvictOldSealersCancelsBeforeDropping fail (this repo's own standard for real cover vs. decorative, cf. D-58's sentinel check) — neither has been run in this pass.` |
| **D-74** | 2 | `jmdn` | **An unauthenticated peer-sampled chain head could gate consensus voting — one lying peer could pull a validator out of quorum.** `thebesync.PeerReporter` (added by `5506c39`, the seedless sync-monitor fallback) sets `SyncStatus.SequencerHead` to the **maximum height claimed by up to 8 sampled peers**, unauthenticated and uncorroborated — one peer's answer sets it. `main.go` fed that same field to `MessagePassing.SetConsensusSyncGate`, where `GateDecision` → `ConsensusVoteEligible` **abstains** once `sequencerHead - localHead > MaxConsensusLagBlocks` (**2**). That gate is consulted on the live vote path (`ListenerHandler.go` `handleVoteResultRequest` — "Abstaining from consensus vote: node not synced") and is enforced by default (`JMDN_ENFORCE_SYNC_GATE != "0"`). **So a peer answering `FetchHead` with `localHead+3` stopped that node voting** — no stake, no committee seat, and a lie small enough to read as ordinary propagation lag: the monitor's delta filter only excuses a gap of 1 (`defaultPropagationToleranceBlocks`) and the consecutive threshold just means answering twice. **Liveness, not safety:** the node abstains rather than voting wrongly, so no forged quorum — but an attacker inside a validator's peer set removes it from quorum silently, and `MaxMainPeers` is small. **Introduced by the producer swap, not the consumer:** before `5506c39` this field came from the seednode, one accountable source; that commit changed who supplies it and left every consumer unchanged. Its own comment calls the substitution *"conservative by construction: at worst it lags the true fleet tip, never ahead of it"* — true of honest peers, false of a lying one | `internal/syncmonitor/head_authenticated_test.go` | S | `Fixed (fix/thebesync-peer-reporter) — found by performing the review 5506c39 shipped without. Merged to v3base as d68eeb6 (PR #157) — cite that SHA, not 17a2dd8: 17a2dd8 was a pre-rebase commit on fix/thebesync-peer-reporter (patch-id-identical to the branch's current 74f37a2) that is not reachable from any ref and will not exist in a fresh clone. 5506c39 (the commit that introduced the bug) is likewise real but reachable only from origin/jmdn-update-audit, not v3base; the precise v3base-reachable introducing commit was not re-identified during this correction (2026-09-25) — treat 5506c39 as descriptive of WHAT introduced the defect, not as a v3base checkout target.` `SyncStatus`/`Status` gained `HeadAuthenticated`: the seednode adapter vouches (`true`), `PeerReporter` does not (`false`), and `main.go`'s `headKnown` now requires it — so an unvouched head takes the documented head-unknown path and permits voting, the same posture as a seednode outage. **Deliberately NOT fixed by zeroing `SequencerHead`** (the first attempt, discarded): the monitor's propagation-lag filter reads the same field, so suppressing it disables that filter and turns every one-block lag into a catch-up. The head is still published for sync tuning and refused for consensus participation — the split is per-CONSUMER, not per-field. **Residual, untested:** `PeerReporter` setting the flag `true` is guarded only by a DO-NOT comment on the literal — `ReportBlockState` needs a live libp2p host and that package has no host harness, so the test covers `Status` propagation (verified by dropping the copy and watching it fail) but not the reporter's own value. **Related fix in the same commit:** `consensus_sync_gate.go` carried a stale *"Default-off"* comment immediately above the `enforceConsensusSyncGate` check, contradicting its own declaration two lines up — RC-3, and precisely the sentence that would talk the next reviewer out of checking |
| **D-75** | 3 | `jmdn` | **`JMDN_TIMEOUT_CERT_WIRING` (pool-wide timeout voting, PR #159 `f8f4cdf`) has two open blockers before it can be flipped on.** **(i)** `ensurePeriodForBlock` (`messaging/timeout_request.go`) runs inside `verifyBlockCertificate` **before** any BLS/committee verification — only `bls_results` non-empty + JSON-parseable is checked first — and on a block claiming a later `Period`, synchronously queries up to `periodCatchUpMaxPeers` (3) peers at `timeoutCertRejoinTimeout` (5s) each: an unauthenticated block with garbage `bls_results` and `Period = local+N` costs up to 15s and 3 outbound streams per malicious block, before the block is ever rejected. **(ii)** `handleTimeoutRequestBroadcast` calls `recordAndMaybeCertify(h, vote, nil)` — the request path can never populate `blockVoters` (it lives in the sequencer's state machine, unreachable from a generic pool node) — so `tryCertify`'s §7.1b block-vote-equivocation exclusion never runs on this path. Before this PR only the sequencer certified (and always had `blockVoters`); after it, every pool node can certify, and none has it — the exclusion goes from always-applied to almost-never-applied. Neither closes the safety property outright — (i) is a bounded liveness/DoS cost, (ii) falls back to layer-1 honest self-restraint (`internal/roundlock`), which does not catch a deliberate Byzantine double-signer | — | M | `Open — found 2026-09-25 reviewing PR #159. F-2 (same-round replay) and F-4 (misleading test name) from the same review are Fixed on the same branch (security/timeout-cert-pool-voting-v3base, commit c3044a5). Do not set JMDN_TIMEOUT_CERT_WIRING=1 until (i) is bounded by a single-flight + claimed-period cap ahead of RoundContextForBlock, and (ii) is either closed or its residual risk is explicitly accepted in writing. RC-2 instance (two-state gate, no shadow rung) — see §7.3.` |
| **D-76** | 4 | `jmdn` | **`require_pinned_committee` (W1) has no shadow rung: the seat fence and the seat draw read the eligible pool from two different functions, and they agree today only because the flag defaults false.** `ResolveMissingSeats`'s `pinnedEligible` comes from `WarmupPeerIDs()` → `eligibleMembersUncapped()` (`committee_v2.go:627`, the LIVE set); the seat draw comes from `committeeSnapshotFor` → `pinnedEligibleForEpoch(epoch)` (`:452`), which with the flag off falls through to that same live call (`:419`) — hence the coincidence. The moment the flag is true, the draw uses the FROZEN epoch snapshot while the fence still checks the live set: a seat from a frozen epoch that has since left the live pool is classified `Unauthorized` and silently dropped — reintroducing the exact missing-seat halt PR #158 exists to fix, with a log line that blames authorization instead of naming the mismatch. Fail direction is closed (a seat is dropped, never an impostor admitted) — a liveness regression, not a safety one | — | S | `Open — found 2026-09-25 reviewing PR #158 (still unmerged, security/critical-remediation-v3base @ 9602250). Fix: fence ResolveMissingSeats against the SAME snapshot the seats were drawn from, and give the Unauthorized path its own WARN naming both set sizes, before require_pinned_committee is ever turned on. Prerequisite for W1. RC-2 instance (two-state gate, no shadow rung) — see §7.3.` |
| **D-77** | 2 | `jmdn` | **A retried vote-result request that fails to sign released the reservation standing behind an EARLIER attempt's shipped signature, re-opening the round to the other side — one node could then sign BOTH a block result and a timeout vote for the same round.** `roundlock.TryLock` is deliberately idempotent for the same side (so a retried vote-result is not refused), and `lockBlockResultRound` collapsed both outcomes to `nil`: the caller could not tell a fresh acquire from a re-entry. The F-3 release path (`f06d0ec`) therefore fired on re-entries too: (1) request reserves round r, (2) its BLS sign SUCCEEDS and the signature ships and counts toward the certificate, (3) a RETRIED request re-enters and gets `ok=true`, (4) that sign FAILS, (5) `Release(r, Block)` compare-and-deletes — the side matches, so the guard does not catch it — (6) the ledger is empty for r and this node may now also sign a timeout vote. That is both sides of one round from one node, which is precisely the mutual exclusion `internal/roundlock` exists to enforce (§7.1b). Reachable on `handleVoteResultRequest` (retries are expected and documented) and on `MaybeStartTimeoutFlow`; NOT on `handleTimeoutRequestBroadcast`, where `actAlreadySigned` returns before `TryLock`. `decideTimeoutRequest` was also exposed by a TOCTOU — its `Signed()` check and its `TryLock` are two separate acquisitions, so a concurrent request for the same round can take the reservation in between. **Latent, not live:** both `SignTimeoutVote` call sites are behind `JMDN_TIMEOUT_CERT_WIRING` (default false, unset in every deploy config), so with the flag off no timeout vote can be produced and the other side cannot be signed. It arms the moment the flag flips | `internal/roundlock/roundlock_test.go`, `AVC/BuddyNodes/MessagePassing/round_lock_test.go` | S | `Fixed — found 2026-09-25 reviewing PR #159, CONFIRMED by a failing test before the fix. roundlock gains TryLockFresh(r, side) (ok, taken, fresh); Release's contract now REQUIRES fresh=true, and all three call sites guard on it (round_lock.go/ListenerHandler.go, timeout_gossip.go, timeout_request.go). F-3's own property is preserved: a round whose ONLY attempt failed to sign is still released (TestF9_FreshReleaseStillFreesAStrandedRound). Regression proof: Release does not exist at f8f4cdf, so D-77 cannot predate f06d0ec. Mutating TryLockFresh to always report fresh fails 4 tests across both packages. NOTE: the 14 pre-existing roundlock tests all PASSED against the defect — the suite did not detect it. RC-6 instance (one primitive, two owners of the invariant).` |
