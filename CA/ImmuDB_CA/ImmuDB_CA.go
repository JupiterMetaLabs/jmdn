...
func EnsureTLSAssets(dir string) error {
	...
	// 4) Client: regen if missing or expired
	clientKeyFile := filepath.Join(dir, "client.key.pem")
	clientCertFile := filepath.Join(dir, "client.cert.pem")
	if err := loadOrCreateClientCert(caCert, caKey, clientCertFile, clientKeyFile, now); err != nil {
		return fmt.Errorf("client cert setup: %w", err)
	}
...
func loadOrCreateClientCert(caCert *x509.Certificate, caKey *ecdsa.PrivateKey, certPath, keyPath string, now time.Time) error {
	// try load existing
	if certPEM, _, err := readPEMFiles(certPath, keyPath); err == nil {
		if cert, err := x509.ParseCertificate(certPEM); err == nil {
			if cert.NotAfter.After(now.Add(24 * time.Hour)) {
	...
	template := x509.Certificate{
		SerialNumber: serial,
		Subject: pkix.Name{
			Organization:  []string{"ImmuDB Client"},
			Country:       []string{"US"},
			Province:      []string{""},
