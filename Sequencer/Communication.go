...
func AskForSubscription(Listener *MessagePassing.StructListener, topic string, consensus *Consensus) error {
	...
	// Make sure responseHandler is cleaned up when we exit this function
	defer cleanupResponseHandler(responseHandler)
	...
	// Verify the returned count matches the slice length (security check)
	if mainAcceptedCount != len(mainAcceptedPeers) {
		log.Printf("WARNING: Mismatch in mainAcceptedCount (%d) vs len(mainAcceptedPeers) (%d) - using slice length", mainAcceptedCount, len(mainAcceptedPeers))
		mainAcceptedCount = len(mainAcceptedPeers)
	}
	...
	if mainAcceptedCount >= config.MaxMainPeers {
		...
		// Proceed only if we have backup peers configured
		if config.MaxBackupPeers > 0 {
			...
			for stillNeeded > 0 && backupPeersIndex < len(allBackupPeers) {
				...
				// Verify batch count sanity
				if batchAcceptedCount != len(batchAcceptedPeers) {
					log.Printf("WARNING: Mismatch in batchAcceptedCount (%d) vs len(batchAcceptedPeers) (%d)", batchAcceptedCount, len(batchAcceptedPeers))
					batchAcceptedCount = len(batchAcceptedPeers)
				}
			...
			if totalAccepted > config.MaxMainPeers {
				log.Printf("WARNING: Got more subscribers than needed (%d > %d) - proceeding with all", totalAccepted, config.MaxMainPeers)
			}
...
func VerifySubscriptions(logger_ctx context.Context, gps *PubSubMessages.GossipPubSub, consensus *Consensus) (map[peer.ID]string, error) {
	...
	// Use the Router to verify subscriptions with a 3-second timeout
	verificationResponses, err := router.VerifySubscriptions(logger_ctx, consensus.PeerList.MainPeers, 3*time.Second)
	if err != nil {
...
func ProcessVerificationMessage(logger_ctx context.Context, gps *PubSubMessages.GossipPubSub, gossipMessage *PubSubMessages.GossipMessage) error {
	// Create Router instance for centralized verification handling
	router := createRouter(gps)
	defer router.Close()
	// Use the Router to process the verification message
	return router.ProcessCompleteVerification(logger_ctx, gossipMessage)
}
...
func SendVerificationResponseWithRouter(logger_ctx context.Context, gps *PubSubMessages.GossipPubSub, accepted bool) error {
	// Create Router instance for centralized verification handling
	router := createRouter(gps)
	defer router.Close()
	// Use the Router to send verification response with validation
	return router.SendVerificationResponseWithValidation(logger_ctx, accepted)
}
...
func askPeersForSubscription(
	...
	for _, peerID := range peerAddrs {
		...
		if err := common.LocalGRO.Go(GRO.SequencerConsensusThread, func(ctx context.Context) error {
			...
			if err := Listener.SendMessageToPeer(timeoutCtx, peerID, string(messageBytes)); err != nil {
