...
func (dw *DataWriter) WriteData(data []*blockpb.NonHeaders) error {
	...
	// Per-block updates are non-deterministic when DataSync workers run
	// concurrently — the last worker to finish may not hold the highest block.
	// A single MONOTONIC update at the end is authoritative (main semantics).
	if didWriteBlock {
		...
		// Record that blocks arrived so the SyncMonitor propagation guard can
		// skip a Merkle check racing with an in-flight data write (main Fix 2).
		notifyBlockReceived()
	}
