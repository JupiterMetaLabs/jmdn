...
type BuddyService struct {
	...
	bftConfig      Config
	signer         Signer
	sequencerCreds credentials.TransportCredentials // Cached client creds for calling back to Sequencer
}
...
func NewBuddyService(
	...
	sequencerURL string,
	tlsCfg *tls.Config, // kept for backward compat, but ignored — creds loaded in StartServer
) *BuddyService {
...
func (s *BuddyService) reportResult(req *pb.BFTRequest, result *Result) {
	...
	if s.sequencerCreds == nil {
		log.Printf("❌ [%s] No sequencer credentials available — skipping report", s.buddyID)
		return
	}

	conn, err := grpc.NewClient(s.sequencerURL, grpc.WithTransportCredentials(s.sequencerCreds))
	if err != nil {
...
func (s *BuddyService) reportFailure(req *pb.BFTRequest, reason string) {
	...
	if s.sequencerCreds == nil {
		log.Printf("❌ [%s] No sequencer credentials available — skipping failure report", s.buddyID)
		return
	}
...
func (s *BuddyService) StartServer(port int) error {
	...
	// 1. Setup Security config and TLS Loader
	secCfg := &settings.Get().Security
	tlsLoader := gatekeeper.NewTLSLoader(secCfg, nil)

	// 2. Load Server TLS Config
	// FAIL HARD: If TLS is enabled in policy but fails to load, we must not start insecurely.
	_, err = tlsLoader.LoadServerTLS(settings.ServiceBFTBuddy)
	if err != nil {
		return fmt.Errorf("failed to load TLS config for BFT Buddy service: %w", err)
	}

	// 3. Cache client credentials for calling back to Sequencer
	// This is loaded once here instead of per-call in reportResult/reportFailure.
	seqCreds, err := tlsLoader.LoadClientCredentials(settings.ServiceBFTSequencer, "bft_buddy")
	if err != nil {
		log.Printf("⚠️ [%s] Failed to load sequencer client credentials: %v (reports will fail)", s.buddyID, err)
	} else {
		s.sequencerCreds = seqCreds
	}

	// Create secure gRPC server via gatekeeper helper
	grpcServer, serverTLS, err := gatekeeper.NewSecureGRPCServer(
		settings.ServiceBFTBuddy, secCfg, nil, false,
	)
	if err != nil {
	...
	if serverTLS != nil {
		...
		log.Printf("⚠️ [%s] BFT gRPC server started WITHOUT TLS (Insecure mode enabled in policy)", s.buddyID)
	}
