...
func TriggerCRDTSyncForBuddyNode(logger_ctx context.Context, listenerNode *AVCStruct.BuddyNode) error {
	...
	err := Connector.SubscribeEnhanced(logger_ctx, pubSubNode.PubSub, topicName, func(gossipMsg *AVCStruct.GossipMessage) {
		...
		logger().Info(context.Background(), "📥 Received CRDT sync from %s (%d/%d buddy nodes)")
	...
	for !subscriptionDone {
		...
		logger().Info(context.Background(), "✅ Merged CRDT from %s (%d/%d merged, %d/%d received, elapsed: %v)")
		...
		logger().Info(context.Background(), "📥 Received from all %d buddies, keeping channel open for %v more to ensure full sync")
		...
		case <-syncComplete:
			elapsed := time.Since(startTime)
		logger().Info(context.Background(), "✅ Received sync messages from all %d buddy nodes (elapsed: %v)")
		...
		case <-timeout:
			...
			receivedMutex.Unlock()
			elapsed := time.Since(startTime)
		logger().Info(context.Background(), "⏱️ Sync duration complete (%v) - received from %d/%d buddy nodes, merged %d")
		...
		if time.Since(lastUpdate) > 2*time.Second && !subscriptionDone {
			...
			remaining := syncDuration - elapsed
			if remaining > 0 {
		logger().Info(context.Background(), "📊 Sync status: %d/%d received, %d merged, %v remaining")
...
func connectToBuddyNodesForSync(listenerNode *AVCStruct.BuddyNode) error {
	...
	if len(buddyTargets) < expectedBuddyCount {
		logger().Info(context.Background(), "⚠️ Only found %d buddy nodes, expected %d (config.MaxMainPeers)")
			len(buddyTargets), expectedBuddyCount)
	}

		logger().Info(context.Background(), "✅ Total buddy nodes to connect: %d (expected: %d from config.MaxMainPeers)")
		len(buddyTargets), expectedBuddyCount)
