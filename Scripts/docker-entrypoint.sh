...
# peer.json — generated by the node on first run.
# Ensure the directory exists with correct ownership so the node can write it.
mkdir -p /opt/jmdn/data/config
chown "${JMDN_USER}:${JMDN_USER}" /opt/jmdn/data/config
