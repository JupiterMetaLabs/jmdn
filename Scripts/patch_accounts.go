...
// every account in accountsdb with the ground-truth Nonce and standardised timestamps.
//
// What gets patched per account:
//   - Nonce     : set to TxCount from replay (= "next expected nonce", i.e. max_nonce + 1 for
//                 well-behaved chains, or exactly TxCount when nonces are 0-indexed and gapless)
...
//
// Usage:
//   go run scripts/patch_accounts.go -host 127.0.0.1 -port 3322 -user immudb -pass immudb
...
func main() {
	...
	for {
		...
		for i := startIndex; i < len(resp.Entries); i++ {
			...
			// ── Nonce correction ─────────────────────────────────────────────────────
			// For addresses that appear in the replay, the true nonce = TxCount.
			// Accounts that have never sent a transaction get nonce = 0.
			var trueNonce uint64 = 0
			if s, exists := stats[acc.Address.Hex()]; exists {
				trueNonce = s.TxCount
			}

			if acc.Nonce != trueNonce {
				acc.Nonce = trueNonce
				fixedNonces++
				needsPatch = true
			}
